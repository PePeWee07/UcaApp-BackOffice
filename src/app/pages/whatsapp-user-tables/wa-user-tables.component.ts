...
import { Content, WhatsAppUserList } from '../../models/catia/WhatsAppUserList';
import { Router, RouterLink } from '@angular/router';
import { AuthService } from '../../core/services/auth/auth.service';
import { UserTicket } from '../../models/catia/WhatsAppUser';
import { TranslateModule, TranslateService } from '@ngx-translate/core';
import { LanguageService } from '../../core/services/language.service';
import { MessageStreamService, MessageUpdateEvent } from '../../core/services/catia/MessageStreamService';
...
export class WhatsAppUserTablesComponent implements OnInit {
  constructor(
    ...
    private stream: MessageStreamService,
  ) {
  ...
  phone = '593983439289'; // <-- pon el waId que estás probando
  pending = 0;
  ...
  ngOnInit(): void {
    ...
    this.stream.connect(this.phone, (ev: MessageUpdateEvent) => {
  ...
  onSearch(event: KeyboardEvent) {
    ...
    if (this.searchText) {
      this.WhatsAppUserListService.getUserInfo(
        this.selectedField,
        this.searchText,
      );
    } else {
  ...
  getWhatsAppUsers() {
    this.WhatsAppUserListService.getWhatsAppUsers(this.url).subscribe({
      next: (res: WhatsAppUserList) => {
        ...
        this.userKeys = Object.keys(this.userList[0]).filter(
          (key: string) => key !== 'chatSessions' && key !== 'erpUser',
        ) as (keyof Content)[];

        // Obtener los keys que se veran en la tabla
        this.tableKeys = [...this.userKeys, ...this.erpUserKeys];
        this.tableKeys = allowedKeys
          .filter((key) => this.tableKeys.includes(key))
          .map((key: string) =>
            key === 'whatsappPhone'
              ? 'Phone Number'
              : key && key === 'conversationState'
                ? 'Conversation State'
                : key && key === 'blockingReason'
                  ? 'Blocking Reason'
                  : key && key === 'nombres'
                    ? 'Name'
                    : key && key === 'apellidos'
                      ? 'Last Name'
                      : key && key === 'identificacion'
                        ? 'Identification'
                        : key && key === 'emailInstitucional'
                          ? 'Email'
                          : key,
          );
        ...
        this.userList[0].erpUser?.rolesUsuario?.forEach((rol) =>
          rol.detallesRol?.forEach((detalle) => this.rolKeys.push(detalle)),
        );
        this.rolKeys = Object.keys(this.rolKeys[0]).map((key: string) =>
          key === 'nombreCarrera'
            ? 'Carrera'
            : key && key === 'unidadAcademica'
              ? 'Unidad Academica'
              : key && key === 'unidadOrganizativa'
                ? 'Unidad Organizativa'
                : key && key === 'nombreRol'
                  ? 'Nombre Rol'
                  : key,
  ...
  getSession(sessionId: number, start: Date, end: Date) {
    this.selectedSession =
      this.selectedSession === sessionId ? 'All' : sessionId;
